...
normalModelCode <- nimbleCode({
  ...
  sigma_observation ~ dnorm(GGt_Default, 100)
  ...
  for (t in 2:T) {
    S[t] ~ dnorm((1 - death_rate[t-1]) * S[t-1] + birth_rate[t-1] * S[t-1], sd = sqrt(S[t-1]))
  }
  
  # Measurement Equation 
  for (t in 1:T) {
    y[t] ~ dnorm(S[t], sd = sigma_observation)
  }
})
...
data <- list(
  ...
  death_rate = Combined_Data$Death_rate,
  birth_rate = Combined_Data$Birth_rate
)
